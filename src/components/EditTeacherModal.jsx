...
const EditTeacherModal = ({ isOpen, onClose, fetchData, selectedTeacher }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`/api/admin/gurustaff/${selectedTeacher.id}`, JSON.stringify(formData), config);
            ...
            <div className="bg-white w-full max-w-3xl p-8 rounded-md shadow-lg max-h-screen overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 relative">
                    <div className="text-center mb-10">
                        <h3 className="text-2xl font-bold tracking-wider text-gray-800">Edit Data Guru</h3>
                    </div>
                    ...
                    <div className="flex justify-end mt-6">
                        <button type="submit" className="px-6 py-2 text-white bg-blue-500 rounded-md shadow-md hover:bg-blue-600 focus:outline-none">Update</button>
                    </div>
                </form>
            </div>
        </div>
